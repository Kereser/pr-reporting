...
public abstract class TemplateAdapterOperations<E, K, V> {
  ...
  protected TemplateAdapterOperations(
    ...
    Class<V> dataClass = (Class<V>) genericSuperclass.getActualTypeArguments()[2];
  ...
  /**
   * @return Mono<List < E>>
   * @implNote Bancolombia does not suggest the Scan function for DynamoDB tables due to the low
   *     performance resulting from the design of the database engine (Key value). Optimize the
   *     query using Query, GetItem or BatchGetItem functions, and if necessary, consider the
  ...
  protected V toEntity(E model) {
    return model != null ? toDataFn.apply(model) : null;
  }
